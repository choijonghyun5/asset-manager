...
const state = {
  ...
  addForm: { name: "", typeKey: null, amount: "", lastMonthAmount: "", date: todayStr(), error: "" },
...
function renderAssetList() {
  ...
  const rows = state.assets
    .map((a) => {
      const value = getCurrentValue(a.id);
  ...
  return `
    <div class="section-label">자산 목록</div>
...
function renderAddScreen() {
  ...
  return `
    ...
    <div class="gap-20"></div>
    <div class="section-label">지난달 금액 (선택)</div>
    <input id="add-last-month-amount" class="input" type="text" inputmode="decimal"
      placeholder="${type && type.currency === "USD" ? "예: 9,500" : "예: 4,800,000"}"
      value="${escapeHtml(f.lastMonthAmount)}" />
    ${type && type.currency === "USD" ? `<div class="helper-text" id="add-last-month-usd-preview"></div>` : ""}
    <div class="helper-text">입력하면 지난달 기록도 함께 저장되어 처음부터 증감·추이를 볼 수 있어요.</div>
...
function updateAddLastMonthUsdPreview() {
  const el = document.getElementById("add-last-month-usd-preview");
  if (!el) return;
  const type = state.addForm.typeKey ? typeByKey(state.addForm.typeKey) : null;
  if (!type || type.currency !== "USD") return;
  const amt = parseAmountInputValue(state.addForm.lastMonthAmount);
  const rate = getEffectiveRate();
  if (!amt || isNaN(amt) || amt <= 0) {
    el.textContent = `현재 환율 ${formatRate(rate)}원이 적용됩니다`;
    return;
...
function bindAddScreenEvents() {
  ...
  const lastMonthAmountInput = document.getElementById("add-last-month-amount");
  ...
  bindAmountInput(lastMonthAmountInput, (formatted) => {
    state.addForm.lastMonthAmount = formatted;
    updateAddLastMonthUsdPreview();
  });
...
function handleAddAssetSubmit() {
  ...
  let lastMonthNumAmount = null;
  if (f.lastMonthAmount.trim() !== "") {
    lastMonthNumAmount = parseAmountInputValue(f.lastMonthAmount);
    if (isNaN(lastMonthNumAmount) || lastMonthNumAmount <= 0) {
      f.error = "지난달 금액을 올바르게 입력해주세요.";
      renderBody();
      bindAddScreenEvents();
      return;
    }
  ...
  if (lastMonthNumAmount !== null) {
    const lastMonthKrwAmount = type.currency === "USD" ? lastMonthNumAmount * rate : lastMonthNumAmount;
    state.records.push({
      id: uid(),
      assetId: asset.id,
      date: addMonthsStr(f.date, -1),
      amount: lastMonthNumAmount,
      exchangeRate: rate,
      krwAmount: lastMonthKrwAmount,
    });
  }

  state.addForm = { name: "", typeKey: null, amount: "", lastMonthAmount: "", date: todayStr(), error: "" };
